...
export function generarPDF(datos, tipo, rango, nombreArchivo = "Reporte.pdf") {
  ...
  img.onload = () => {
    ...
    if ((tipo === "cortes" || tipo === "reconexiones") && rango.start && rango.end) {
      // ... (igual que antes, no modifico esta parte)
      // Logo y encabezado
      ...
      const registrosPorDia = Object.entries(grouped).map(([fecha, arr]) => ({
        fecha,
        cantidad: arr.length
      }));
    ...
    // ============ MODIFICACIÓN PARA TENDENCIA =============
      ...
      // 2. Hojas detalladas por día (si tienes el detalle por día, agrega aquí)
